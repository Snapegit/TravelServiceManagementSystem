	...
	<div class="app-contain" :style='{"width":"100%","padding":"0 18%","margin":"0","overflow":"hidden","background":"#fff"}'>
		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			...
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('lvyouluxian','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
			...
			<div class="data_box">
				<div class="data_view">
					<div class="data_item animation_box" v-for="(item,index) in list" :key="index" @click.stop="detailClick(item.id)" >
						<div class="data_img_box" v-if="item.luxiantupian && item.luxiantupian.substr(0,4)=='http'" @click.stop="preViewClick(item.luxiantupian)">
							<el-image class="data_img" :src="item.luxiantupian" fit="cover"></el-image>
						</div>
						<div class="data_img_box" v-else @click.stop="preViewClick($config.url+item.luxiantupian.split(',')[0])">
							<el-image class="data_img" :src="item.luxiantupian?$config.url + item.luxiantupian.split(',')[0]:''"
								fit="cover"></el-image>
						</div>
						<div class="data_content">
							<div class="data_title">路线名称：{{item.luxianmingcheng}}</div>
...
<script setup>
	...
	const addClick = () => {
		router.push('/index/lvyouluxianAdd')
	}
	//判断是否从个人中心跳转
	...
	const detailClick = (id) => {
		router.push('lvyouluxianDetail?id=' + id + (centerType.value?'&&centerType=1':''))
	}
	//下载文件
	const download = (file) =>{
		if(!file){
