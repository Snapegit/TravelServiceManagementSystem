		...
		<el-dialog v-model="formVisible" :title="formTitle" width="80%" destroy-on-close :fullscreen='false'>
			<el-form class="formModel_form" ref="formRef" :model="form" label-width="$template2.back.add.form.base.labelWidth" :rules="rules">
				<el-row>
					...
					<el-col :span="12">
						<el-form-item label="景点分类" prop="jingdianfenlei">
							<el-select
								class="list_sel"
								:disabled="!isAdd||disabledForm.jingdianfenlei?true:false"
								v-model="form.jingdianfenlei" 
								placeholder="请选择景点分类"
								>
								<el-option v-for="(item,index) in jingdianfenleiLists" :label="item"
									:value="item"
									>
								</el-option>
							</el-select>
					...
					<el-col :span="12">
						<el-form-item label="购票须知" prop="goupiaoxuzhi">
							<el-input v-model="form.goupiaoxuzhi" placeholder="购票须知" type="textarea"
							:readonly="!isAdd||disabledForm.goupiaoxuzhi?true:false"
							/>
						</el-form-item>
					</el-col>
					<el-col :span="12">
...
<script setup>
	...
	const getInfo = ()=>{
		context?.$http({
			...
			res.data.data.jingdianjieshao = res.data.data.jingdianjieshao?(res.data.data.jingdianjieshao.replace(reg,'../../../cl9446836/file')):'';
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		...
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='jingdianfenlei'){
					form.value.jingdianfenlei = row[x];
					disabledForm.value.jingdianfenlei = true;
					continue;
				}
				if(x=='jingdiantupian'){
				...
				if(x=='goupiaoxuzhi'){
					form.value.goupiaoxuzhi = row[x];
					disabledForm.value.goupiaoxuzhi = true;
					continue;
				}
				if(x=='jingdiandizhi'){
		...
		context?.$http({
			...
			jingdianfenleiLists.value = res.data.data
		})
	}
	//初始化
	//声明父级调用
